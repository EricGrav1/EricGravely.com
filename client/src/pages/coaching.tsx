      ...
      <main className="pt-24">
        ...
        >
          <div className="relative max-w-4xl mx-auto px-4 sm:px-6 text-center">
            ...
            >
              <span className="label-track block mb-6">
                {coaching.badge}
              </span>
              <h1 className="font-display text-4xl md:text-6xl font-bold mb-6 leading-tight" style={{ color: "var(--c-fg)" }}>
                {coaching.pageHeroTitle}<br />
                <span className="gold-underline">{coaching.pageHeroTitleHighlight}</span>
              </h1>
        ...
        <section className="py-20 md:py-28" style={{ background: "var(--c-bg2)" }}>
          <div className="max-w-6xl mx-auto px-4 sm:px-6">
            ...
            >
              ...
              <h2 className="font-display text-3xl md:text-4xl font-bold" style={{ color: "var(--c-fg)" }}>
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
              {coaching.modules.map((mod, i) => (
                ...
                >
                  <div className="font-display font-bold text-5xl mb-4 leading-none" style={{ color: "var(--c-accent-15)" }}>
                    {mod.number}
                  </div>
                  <h3 className="font-display font-semibold text-lg mb-2" style={{ color: "var(--c-fg)" }}>{mod.title}</h3>
                  <p className="text-sm leading-relaxed mb-4" style={{ color: "var(--c-fg-45)" }}>{mod.description}</p>
                  <ul className="space-y-1.5">
                    {mod.topics.map((topic, j) => (
                      <li key={j} className="flex items-start gap-2 text-xs" style={{ color: "var(--c-fg-45)" }}>
                        <span style={{ color: "var(--c-accent)" }} className="mt-0.5">→</span>
                        {topic}
                      </li>
                    ))}
                  </ul>
        ...
        <section className="py-20" style={{ background: "var(--c-bg)" }}>
          <div className="max-w-4xl mx-auto px-4 sm:px-6">
            ...
            <div className="grid md:grid-cols-2 gap-8">
              ...
              >
                <h3 className="font-display font-semibold text-lg mb-5" style={{ color: "var(--c-accent)" }}>{coaching.forTitle}</h3>
              ...
              >
                <h3 className="font-display font-semibold text-lg mb-5" style={{ color: "var(--c-fg-55)" }}>{coaching.notForTitle}</h3>
                <ul className="space-y-4">
