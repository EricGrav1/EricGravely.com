...
function shouldShowPriceIncrease(): boolean {
  ...
  const increaseDate = new Date(PRICE_INCREASE_DATE);
  const now = new Date();
  
  return increaseDate > now;
}

function generateEmailHtml(email: string, baseUrl: string): string {
...
<head>
  ...
  <title>Your Self Coaching Matrix</title>
</head>
  ...
  <div style="max-width: 600px; margin: 0 auto; padding: 40px 20px;">
    <div style="background-color: #ffffff; border-radius: 12px; padding: 40px; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1);">
      ...
      <h1 style="font-size: 28px; font-weight: 700; color: #111827; margin: 0 0 24px 0; text-align: center;">
        Your Self Coaching Matrix
      </h1>
      
      <p style="font-size: 16px; color: #4b5563; margin-bottom: 32px; text-align: center;">
        Thank you for your interest! Here's your copy of the Self Coaching Matrix. Use it to transform your sales conversations.
      </p>

      ${MATRIX_URL ? `
      <div style="text-align: center; margin-bottom: 24px;">
        <a href="${MATRIX_URL}" style="display: inline-block; background-color: #3b82f6; color: #ffffff; text-decoration: none; padding: 16px 32px; border-radius: 8px; font-weight: 600; font-size: 16px;">
          Download the Self Coaching Matrix (PDF)
        </a>
      </div>
      ` : ''}
    ...
    <div style="text-align: center; padding: 24px 0;">
      <p style="color: #9ca3af; font-size: 12px; margin: 0;">
        You're receiving this email because you requested the Self Coaching Matrix.
      </p>
...
export async function sendLeadMagnetEmail(email: string, baseUrl: string): Promise<{ success: boolean; error?: string }> {
  try {
    ...
    const html = generateEmailHtml(email, baseUrl);

    await client.emails.send({
      from: fromEmail,
      to: email,
      subject: "Your Self Coaching Matrix",
      html,
    });
